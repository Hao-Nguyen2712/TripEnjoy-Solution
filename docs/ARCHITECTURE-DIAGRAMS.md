...
> **📘 Comprehensive Documentation**: For detailed ERD documentation including all 23 entities, business rules, and relationships, see [DATABASE-ERD.md](./DATABASE-ERD.md)

### Entity Relationship Diagram (Complete Overview)

```
┌──────────────────────────────────────────────────────────────────────────┐
│                          TRIPENJOY DATABASE SCHEMA                        │
│                            (23 Main Entities)                             │
└──────────────────────────────────────────────────────────────────────────┘
...
└────────┬─────────┘
         ...
         │             │                │
    ┌────▼──────┐ ┌───▼──────────┐ ┌──▼─────────┐
    │RefreshToken│BlackListToken│ │ AuditLog   │
    └───────────┘ └──────────────┘ └────────────┘
...
└────────┬─────────┘         └────────┬─────────┘
      ...
      1:N  │      1:N  │      1:N  │      1:N  │
  ┌────────▼──┐  ┌────▼────────┐ ┌▼─────────┐ ┌▼──────────┐
  │RoomType   │  │   Room      │ │  Room    │ │  Voucher  │
  │  Image    │  │Availability │ │Promotion │ │  Target   │
  └───────────┘  └─────────────┘ └──────────┘ └───────────┘
                                                 (ROOM)
...
1. Account Aggregate (Root: Account)
   ├── User, Partner, PartnerDocument
   ├── Wallet (with Transaction, Settlement)
   ├── RefreshToken, BlackListToken
   └── AuditLog

2. Property Aggregate (Root: Property)
   ├── PropertyImage
   └── RoomType
       ├── RoomTypeImage
       ├── RoomAvailability
...
3. Booking Aggregate (Root: Booking)
   ├── BookingDetail
   ├── BookingHistory
   ├── Payment
   └── BookingVoucher

4. Review Aggregate (Root: Review)
   ├── ReviewImage
   └── ReviewReply

5. Voucher Aggregate (Root: Voucher)
   └── VoucherTarget

6. PropertyType Aggregate (Root: PropertyType)

7. AuditLog Aggregate (Root: AuditLog)
```
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│  ✅ IMPLEMENTED TABLES (Current - 11 tables)                             │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  ACCOUNT MANAGEMENT (7 tables):                                          │
│  • Accounts (AccountId PK, AspNetUserId FK, Email, IsDeleted, ...)      │
│  • Users (UserId PK, AccountId FK, FullName, Phone, DateOfBirth, ...)   │
│  • Partners (PartnerId PK, AccountId FK, CompanyName, Status)            │
│  • PartnerDocuments (DocId PK, PartnerId FK, Type, Status, FilePath)    │
│  • Wallets (WalletId PK, AccountId FK, Balance)                         │
│  • RefreshTokens (TokenId PK, AccountId FK, Token, ExpiryDate, ...)     │
│  • BlackListTokens (TokenId PK, AccountId FK, Token, ExpiredAt)         │
│                                                                          │
│  PROPERTY MANAGEMENT (3 tables):                                         │
│  • Properties (PropertyId PK, PartnerId FK, PropertyTypeId FK, ...)     │
│  • PropertyImages (ImageId PK, PropertyId FK, FilePath, IsMain)         │
│  • PropertyTypes (TypeId PK, Name, Status)                              │
│                                                                          │
│  AUDIT & IDENTITY (1 table + ASP.NET Identity):                         │
│  • AuditLogs (LogId PK, AccountId FK, Action, EntityName, ...)          │
│  • AspNetUsers, AspNetRoles, AspNetUserRoles (ASP.NET Identity)         │
└─────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────┐
│  ❌ PLANNED TABLES - NOT YET IMPLEMENTED (12 tables)                     │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  🔴 HIGH PRIORITY - ROOM MANAGEMENT (4 tables):                          │
│  • RoomTypes (RoomTypeId PK, PropertyId FK, Name, Capacity, Price, ...) │
│  • RoomTypeImages (ImageId PK, RoomTypeId FK, FilePath, IsMain)         │
│  • RoomAvailabilities (AvailId PK, RoomTypeId FK, Date, Quantity, ...)  │
│  • RoomPromotions (PromoId PK, RoomTypeId FK, Discount%, StartDate, ...) │
│                                                                          │
│  🔴 HIGH PRIORITY - BOOKING SYSTEM (4 tables):                           │
│  • BookingDetails (DetailId PK, BookingId FK, RoomTypeId FK, ...)       │
│  • BookingHistories (HistoryId PK, BookingId FK, Status, ChangedAt, ...)│
│  • Payments (PaymentId PK, BookingId FK, Amount, Method, Status, ...)   │
│  • BookingVouchers (BVId PK, BookingId FK, VoucherId FK, Discount)      │
│                                                                          │
│  🟡 MEDIUM PRIORITY - REVIEW SYSTEM (3 tables):                          │
│  • Reviews (ReviewId PK, BookingDetailId FK, UserId FK, Rating, ...)    │
│  • ReviewImages (ImageId PK, ReviewId FK, FilePath)                     │
│  • ReviewReplies (ReplyId PK, ReviewId FK, ReplierType, Content, ...)   │
│                                                                          │
│  🟡 MEDIUM PRIORITY - VOUCHER SYSTEM (3 tables):                         │
│  • Vouchers (VoucherId PK, Code UNIQUE, Type, Value, Limit, ...)        │
│  • VoucherTargets (TargetId PK, VoucherId FK, TargetType, TargetId)    │
│  • (BookingVouchers - listed above in Booking System)                   │
│                                                                          │
│  🟢 LOW PRIORITY - FINANCIAL ENHANCEMENT (2 tables):                     │
│  • Transactions (TxId PK, WalletId FK, BookingId FK, Amount, Type, ...) │
│  • Settlements (SettleId PK, WalletId FK, Period, Amount, Commission)   │
└─────────────────────────────────────────────────────────────────────────┘
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  Phase 1: Room Management (Required for Booking)                        │
│  - RoomTypes, RoomTypeImages, RoomAvailabilities, RoomPromotions        │
│                                                                          │
│  Phase 2: Enhanced Booking (Core Revenue Driver)                        │
│  - BookingDetails, BookingHistories, Payments, BookingVouchers          │
│                                                                          │
│  Phase 3: Financial Enhancement (Partner Payouts)                       │
│  - Transactions, Settlements                                            │
│                                                                          │
│  Phase 4: Review System (Trust & Quality)                               │
│  - Reviews, ReviewImages, ReviewReplies                                 │
│                                                                          │
│  Phase 5: Voucher System (Marketing & Promotions)                       │
│  - Vouchers, VoucherTargets                                             │
└─────────────────────────────────────────────────────────────────────────┘
```
...
│ Metric                         │ Count    │
├────────────────────────────────┼──────────┤
│ Total Tables Implemented       │    11    │
│ Total Tables Planned           │    23    │
│ Implementation Progress        │   48%    │
│ Aggregates Implemented         │   4/7    │
│ Domain Entities Complete       │   11     │
│ Missing Core Entities          │   12     │
└────────────────────────────────┴──────────┘

Entity Breakdown by Aggregate:
┌────────────────────────────────┬──────────┬──────────┐
│ Aggregate                      │ Status   │ Entities │
├────────────────────────────────┼──────────┼──────────┤
│ Account Aggregate              │    ✅    │   7/7    │
│ Property Aggregate             │    ⚠️    │   3/7    │
│ PropertyType Aggregate         │    ✅    │   1/1    │
│ Booking Aggregate              │    ❌    │   0/5    │
│ Review Aggregate               │    ❌    │   0/3    │
│ Voucher Aggregate              │    ❌    │   0/3    │
│ Financial Aggregate            │    ⚠️    │   1/3    │
